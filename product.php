...
<h3 class="card-title">  <div class="form-group ">
                  <label for="exampleSelectBorder"><code>Filter By Category</code></label>
                  <select class="form-control " name="category"  onchange="category(event)" id="exampleSelectBorder">
...
$sql = "SELECT 
    p.*,
    p.stock - IFNULL(SUM(o.quantity), 0) AS stock,
    (p.price - p.purchase) AS profit
FROM 
    product p
LEFT JOIN 
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        // Determine the status badge based on the stock quantity
        if ($stock <= 0) {
            $status = '<span style="font-size:14px;" class="badge bg-danger"><i class="fa-solid fa-circle-exclamation"></i> Out of Stock(s)</span>';
        } else {
            $status = '<span style="font-size:14px;" class="badge bg-success">' . $stock . ' In Stock</span>';
        }
...
<td>'.$row['supplier'].'</td>
<td class="text-bold" >₱ '.$row['purchase'].'</td>
<td class="text-bold" >₱ '.$row['price'].'</td>
<td class="text-bold" >₱ '.$row['profit'].'</td>
...
<div class="modal fade" id="del'.$row['id'].'" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    ...
    <center> <h2><strong> Delete Product ?? <strong> </h2> </center>
         <form style="display:none;" method="post" action="product.php">
                      ...
                      <input style="display:none;" type="text" name="product_name" value="'.$row['product_name'].'">
     ...
     <hr>
      ...
      <center>   <img src="mobile-app/product/'.$row['image'].'" width="150px">  </center>
                ...
                <div class="form-group">
                  ...
                  <div class="form-group">
                    <label for="dd">Purchase Price</label>
                    <input type="number" class="form-control" id="dd" required name="purchase" min="1"    value="'.$row['purchase'].'">
                  </div>
            ...
            <input type="text" class="form-control" id="supplier" required name="supplier">
                  ...
                  <div class="form-group">
                    <label for="dd">Selling Price</label>
...
$activity = ' Deleted    '.$product_name.' Product ';
...
$uid = $_SESSION['id'];
$fullname = $_SESSION['fullname'];
$activity = ' Updated    '.$product_name.' Product ';
$sql = "INSERT INTO logs (user_id, fullname, activity, month, day, year, time)
VALUES ('$uid', '$fullname', '$activity', '$month', '$day', '$year', '$time')";
...
$sql = "UPDATE product SET product_name ='$product_name', description='$description',supplier ='$supplier', price='$price', category='$category', stock='$stock', purchase='$purchase', image='$file_name' WHERE id='$id'";
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <form method="post" action="product.php" enctype="multipart/form-data">
          ...
          <div class="form-group">
            <label for="dd">Purchase Price</label>
            <input type="number" class="form-control" id="dd" required name="purchase" min="1">
          </div>
          <div class="form-group">
            <label for="dd">Supplier Name</label>
            <input type="text" class="form-control" id="dd" required name="supplier">
          </div>
          <div class="form-group">
...
if ($conn->query($sql) === TRUE) {
 // echo "New record created successfully";
  ...
  $file_new_name = date("dmy") . time() . $_FILES["file"]["name"]; // New and unique name of uploaded file
    ...
    $sql = "INSERT INTO product (product_name, description,purchase, supplier, price, category, stock, image)
VALUES ('$product_name', '$description', '$purchase', '$supplier','$price', '$category', '$stock', '$file_name' )";
...
<script>
  $(document).ready(function() {
    $('#example1').DataTable({
      dom: 'Blfrtip',
      lengthMenu: [ [25, 1000, 10000, 5000], [25, 1000, 10000, 5000] ], // Show options for 25, 1000, 10000, 5000 entries
      buttons: [
        {
          extend: 'excelHtml5',
          exportOptions: {
