...
<head>
  ...
  <title>Manage User | Sembrano Store</title>
...
if ($result->num_rows > 0) {
  ...
  while($row = $result->fetch_assoc()) {
    ...
    $coop_id = $row['coop_id'];
    if($verify == 0){
      $status = '<span style="font-size:14px;" class="badge bg-danger"><i class="fa-solid fa-circle-exclamation"></i> Unverified</span>';
    }

    if($verify == 1){
      $status = '<span style="font-size:14px;"  class="badge bg-success"><i class="fa-solid fa-certificate"></i> Verified</span>';
      $as = '<span style="font-size:14px;"  class="badge bg-success"><i class="fa-solid fa-user-check"></i> '. $row['status'].'</span>';
    }
    ...
    if($coop_id != ''){
      $status2 = '<span style="font-size:14px;"  class="badge bg-warning"><i class="fa-solid fa-certificate"></i> COOP Member</span>';
    ...
    <center> <h2><strong>  Are you sure to delete ?? <strong> </h2> </center>
         <form style="display:none;" method="post" action="user.php">
                      ...
                      <input style="display:none;" type="text" name="id" value="'.$row['id'].'">
                       <input type="hidden" class="form-control" id="dd" readonly name="fullname1"  value="'.$row['fullname'].'">
...
<div class="modal fade" id="ver'.$row['id'].'" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
    <center> <h2><strong class="text-success"> Verify User Account   </strong> </h2> </center>
         <form style="display:none;" method="post" action="user.php">
          
                  <div class="form-group">
                    <label for="dd">Fullname</label>
                    <input type="text" class="form-control" id="dd" readonly name="fullname1"  value="'.$row['fullname'].'">
                  </div>


                  <div class="form-group">
                    <label for="dd">Contact</label>
                    <input type="text" class="form-control" id="dd" readonly  value="'.$row['contact'].'">
                  </div>


                  <div class="form-group">
                    <label for="dd">Fullname</label>
                    <input type="text" class="form-control" id="dd" readonly  value="'.$row['email'].'">
                  </div>


                
                <div class="form-group">
                  <label for="dd"><code>Verify user as </code></label>
                  <select name="status" class="custom-select " id="dd">
                ...
                ID : 
                <img src="mobile-app/images/'.$row['file'].'" width="200px"><br>

                   <a   href="mobile-app/images/'.$row['file'].'" download="mobile-app/images/'.$row['file'].'">  Click here to download uploaded ID </a>
                    
                    <input style="display:none;" type="text" name="id" value="'.$row['id'].'">
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
        <button type="submit" name="ver" class="btn btn-success">Verify</button>
      </div>
      </form>
    </div>
  </div>
...
$activity = ' Deleted    '.$fullname1.' User Account ';
...
$status = $_POST['status'];
   $fullname1 = $_POST['fullname'];
$sql = "UPDATE user SET status ='$status', verify=1 WHERE id='$id'";
...
$uid = $_SESSION['id'];
$fullname = $_SESSION['fullname'];
$activity = ' Successfully Verified '.$fullname1.' User Account ';
$sql = "INSERT INTO logs (user_id, fullname, activity, month, day, year, time)
VALUES ('$uid', '$fullname1', '$activity', '$month', '$day', '$year', '$time')";
